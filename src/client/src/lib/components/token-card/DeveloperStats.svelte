<script lang="ts">
    import DevWalletLabel from "$lib/components/token-card/DevWalletLabel.svelte";
...
<div class="mt-0.5 flex flex-col gap-1.5">
    <div class="flex w-full flex-wrap items-center gap-2">
        <div class="flex min-w-0 grow flex-wrap items-center gap-1.5">
            ...
            <span
                class={[
                    "rounded border border-purple-500/30 bg-purple-500/15",
                    "px-1.5 py-0.5 text-[0.7rem] font-semibold",
                    "whitespace-nowrap text-purple-400 uppercase",
                ]}>{feed.blockchain || "sol"}</span
            >
        </div>

        <DevWalletLabel wallet={feed.dev_wallet} />

        {#if feed.dev_holds_percent !== null}
            <span
                class={[
                    "ml-auto inline-flex h-6 shrink-0 items-center rounded border",
                    "border-amber-500/30 bg-amber-500/15 px-1.5 text-[0.7rem]",
                    "font-semibold tracking-[0.02em] whitespace-nowrap text-amber-400",
    ...
    <div
        class={[
            ...
            "border-white/[0.045] bg-black/10 px-2 py-1.5 text-[0.8rem]",
