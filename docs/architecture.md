...
предсказуемый URL Axiom CDN. При ошибке загрузки компонент немедленно переходит к
следующему кандидату. Для свежего токена последний кандидат повторяется с
ограниченной задержкой; предыдущие токены не создают повторный сетевой трафик.
До успешной загрузки остается видимым текстовый fallback с тикером.
